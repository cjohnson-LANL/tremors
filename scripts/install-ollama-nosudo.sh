...
# THIS CODE WAS EDITED FROM https://ollama.com/ (curl -fsSL https://ollama.com/install.sh | sh)
...
usage() {
    ...
    echo ""
    echo "Options:"
    echo "  -d, --install-dir <path>   Installation directory (default: \$HOME/ollama)"
    echo "  -h, --help                 Show this help message"
    echo ""
    echo "Examples:"
    echo "  $0 --install-dir /projects/mygroup/ollama"
    echo "  $0 -d /home/username/ollama"
    echo "  $0   # installs to \$HOME/ollama"
    exit 0
}
...
# Default install directory if not specified
if [ -z "$OLLAMA_INSTALL_DIR" ]; then
    OLLAMA_INSTALL_DIR="$HOME/ollama"
    status "No install directory specified. Defaulting to $OLLAMA_INSTALL_DIR"
    status "Tip: Use --install-dir <path> to specify a custom location."
else
    status "Installing to: $OLLAMA_INSTALL_DIR"
fi
...
status "Standalone installation finished successfully."
status "To use ollama, add it to your PATH by running:"
status "  export PATH=\$PATH:$BINDIR"
status ""
status "Then start the server manually in the background:"
status "  ollama serve &"
